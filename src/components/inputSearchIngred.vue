<template>
  ...
  >
    ...
    <input
      ...
      :placeholder="
        !checkProducts ? 'Введите ингредиент' : 'Данный ингредиент отсутствует'
      "
    />
    <button @click="saveProdBtn()" class="bg-yellow-500 w-28 h-6 rounded-3xl ">
      Добавить
    </button>
  </div>
  <div
  ...
  >
    <h1
      ...
      v-for="(item, index) in filterProducts.slice(0, 6)"
      :key="index"
      class="border border-black border-opacity-25 rounded-3xl bg-yellow-500 px-5 h-6 cursor-pointer"
    >
      {{ item }}
    </h1>
  </div>
...
export default {
  ...
  computed: {
    filterProducts() {
      return this.$store.getters.filterProducts(this.search.toLowerCase());
    },
  },
  created() {
