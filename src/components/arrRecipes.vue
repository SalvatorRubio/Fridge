<template>
    <div class="flex flex-col justify-between w-full px-12">
        <h1 class="text-2xl pl-4 font-medium">Найдено для вас {{countRecipe}} 
            <span v-if="countRecipe == 1">рецепт!</span>
            <span v-else-if="countRecipe > 1 && countRecipe < 5">рецепта!</span>
            <span v-else>рецептов!</span></h1>
        <div v-if="arrDataRecipes.length" class="flex flex-wrap justify-between mt-2">
            <div 
            v-for="item in arrDataRecipes"
            :key="item.id"
            class="w-96 h-40 border border-gray-400 rounded-3xl cursor-pointer hover:shadow transition-colors flex justify-evenly items-center m-2">
                ...
                <div class="flex flex-col justify-between">
                    ...
                    <div class="flex items-center my-1">
                        <img class="w-5 h-5 mr-1" src="../assets/recipe/clock.png" alt="">
                        <h1>{{item.time}}</h1>
                    </div>
                    <div class="flex items-center my-1">
